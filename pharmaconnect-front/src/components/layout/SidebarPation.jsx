...
const SidebarPation = ({ isOpen, onClose }) => {
  ...
  const displayName =
    user?.name || [user?.prenom, user?.nom].filter(Boolean).join(" ") || "Patient";
    ...
    <>
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto py-2">
          <NavSection label="Patient" />
          <NavItem to="/patient" icon={<Home size={16} />} label="Tableau de bord" />
          ...
          <NavItem to="/patient/settings" icon={<Settings size={16} />} label="Paramètres" />
        </nav>
        ...
        <div className="border-t border-white/10 p-4 flex items-center gap-3 flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-white/50 leading-tight">Patient</p>
          </div>
          <button
