...
const SidebarDoctor = ({ isOpen, onClose }) => {
  ...
  const displayName =
    user?.name || [user?.prenom, user?.nom].filter(Boolean).join(" ") || "Docteur";
    ...
    <>
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto py-2">
          <NavSection label="Médecin" />
          <NavItem to="/docteur" icon={<LayoutDashboard size={16} />} label="Tableau de bord" />
          <NavItem to="/docteur/rendezvous" icon={<CalendarDays size={16} />} label="Rendez-vous" />
          <NavItem to="/docteur/patients" icon={<Users size={16} />} label="Patients & fiches" />
          <NavItem to="/docteur/ordonnances" icon={<FileText size={16} />} label="Ordonnances" />

          <NavSection label="Module IA" />
          <NavItem to="/medecin/analyse" icon={<BrainCircuit size={16} />} label="Analyse IA" />

          <NavSection label="Gestion" />
          <NavItem to="/docteur/secretaires" icon={<UserRoundPlus size={16} />} label="Mes secrétaires" />
          <NavItem to="/docteur/profile" icon={<User size={16} />} label="Profil" />
          <NavItem to="/docteur/settings" icon={<Settings size={16} />} label="Paramètres" />
        </nav>
        ...
        <div className="border-t border-white/10 p-4 flex items-center gap-3 flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-white/50 leading-tight">Médecin</p>
          </div>
          <button
