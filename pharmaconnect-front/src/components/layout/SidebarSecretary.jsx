...
const SidebarSecretary = ({ isOpen, onClose }) => {
  ...
  const displayName =
    user?.name || [user?.prenom, user?.nom].filter(Boolean).join(" ") || "Secrétaire";
    ...
    <>
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto py-2">
          <NavSection label="Secrétariat" />
          <NavItem to="/secretaire/rendezvous" icon={<CalendarDays size={16} />} label="Rendez-vous" />
          <NavItem to="/secretaire/patients" icon={<Users size={16} />} label="Patients" />
          <NavItem to="/secretaire/ordonnances" icon={<FileText size={16} />} label="Ordonnances" />

          <NavSection label="Compte" />
          <NavItem to="/secretaire/profile" icon={<UserRound size={16} />} label="Profil" />
          <NavItem to="/secretaire/settings" icon={<Settings size={16} />} label="Paramètres" />
        </nav>
        ...
        <div className="border-t border-white/10 p-4 flex items-center gap-3 flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-white/50 leading-tight">Secrétaire</p>
          </div>
          <button
