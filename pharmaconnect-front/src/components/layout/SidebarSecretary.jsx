...
import { CalendarDays, FileText, LogOut, Settings, UserRound, Users } from "lucide-react";
...
const SidebarSecretary = ({ isOpen, onClose }) => {
  const { user, logout } = useAuth();
  const navigate = useNavigate();
  const displayName = user?.name || [user?.prenom, user?.nom].filter(Boolean).join(" ") || "Secretaire";
      ...
      >
        <div className="flex h-full flex-col p-4">
          ...
          <nav className="mt-6 flex-1 space-y-2">
            <SidebarItem to="/secretaire/rendezvous" icon={<CalendarDays size={18} />} label="Rendez-vous" />
            <SidebarItem to="/secretaire/patients" icon={<Users size={18} />} label="Patients" />
            <SidebarItem to="/secretaire/ordonnances" icon={<FileText size={18} />} label="Ordonnances" />
            <SidebarItem to="/secretaire/profile" icon={<UserRound size={18} />} label="Profil" />
            <SidebarItem to="/secretaire/settings" icon={<Settings size={18} />} label="Parametres" />
          </nav>
