...
export const addPharmacy = async (req, res) => {
  ...
  try {
    ...
    const [userResult] = await connection.execute(
      "INSERT INTO users (email, password, role) VALUES (?, ?, 'pharmacist')",
    ...
    return res.status(201).json({
      ...
      user: { id: userId, email, role: "pharmacist" },
    });
  } catch (error) {
    await connection.rollback();
...
export const deletePharmacy = async (req, res) => {
  ...
  try {
    ...
    // Get user_id — cascade on FK handles pharmacie deletion
    ...
    await connection.execute("DELETE FROM users WHERE id = ?", [pharmRows[0].user_id]);
...
export const togglePharmacyStatus = async (req, res) => {
  try {
    ...
    if (pharmacy.length === 0) return res.status(404).json({ error: "Pharmacie non trouvee" });
...
export const getPharmacyDetails = async (req, res) => {
  try {
    ...
    if (rows.length === 0) return res.status(404).json({ error: "Pharmacie non trouvee" });
...
export const getAdminProfile = async (req, res) => {
  try {
    ...
    if (rows.length === 0) return res.status(404).json({ error: "Admin non trouve" });
...
export const updateAdminProfile = async (req, res) => {
  ...
  try {
    ...
    const [adminRows] = await db.execute(
      "SELECT id, user_id, email FROM admin WHERE id = ?",
      [adminId],
    );
    if (adminRows.length === 0) return res.status(404).json({ error: "Admin non trouve" });
    ...
    // Sync auth account via user_id FK when email changes
    if (email !== adminRows[0].email) {
      await connection.execute(
        "UPDATE users SET email = ? WHERE id = ?",
        [email, adminRows[0].user_id],
      );
    }
...
export const changeAdminPassword = async (req, res) => {
  try {
    ...
    // Get user_id from admin — password lives only in users table
    const [adminRows] = await db.execute(
      "SELECT user_id FROM admin WHERE id = ?",
      [adminId],
    );
    if (adminRows.length === 0) return res.status(404).json({ error: "Admin non trouve" });
